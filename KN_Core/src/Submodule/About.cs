...
namespace KN_Core {
  ...
  public class About : BaseMod {
    ...
    private void GuiAbout(Gui gui, ref float x, ref float y, float width, float height) {
      ...
      string supportersText = Locale.Get("about6");
      float linkWidth = Gui.TextWidth(supportersText, Skin.ButtonLeftLinkSkin.Normal.button.font);
      float fillWidth = width - linkWidth;
      float dummyX = x;
      float dummyY = y;

      if (gui.TextButton(ref dummyX, ref dummyY, linkWidth, height, supportersText, Skin.ButtonLeftLinkSkin.Normal)) {
        Process.Start("https://github.com/trbflxr/kino/blob/master/Supporters.md");
      }
      gui.BoxAutoWidth(linkWidth + x, y, fillWidth, height, "", Skin.BoxLeftSkin.Normal);
      y += height;
