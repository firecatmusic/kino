      ...
      using (var client = new WebClient()) {
        ...
        try {
          ...
          Log.Write(module.Length > 0
            ? $"[KN_Loader::WebLoader]: Unable to load data for module '{module}' from remote ({uri}), {e.Message}"
            : $"[KN_Loader::WebLoader]: Unable to load data from remote ({uri}), {e.Message}");
          return null;
        }
      }
    }
    ...
    public static byte[] DownloadNewUpdater() {
      ...
      using (var client = new WebClient()) {
        try {
          ...
          Log.Write($"[KN_Loader::WebLoader]: Failed to download new updater from, {e.Message}");
          return null;
        }
      }
    }
